...
class MechanicalWorkSheetScreen extends StatefulWidget {
  @override
  State<StatefulWidget> createState() {
    return _MechanicalWorkSheetScreenState();
  }
}

class _MechanicalWorkSheetScreenState extends State<MechanicalWorkSheetScreen> {
  @override
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) {
      Provider.of<MechanicalWorkSheetModel>(context, listen: false)
          .getMechanicalWorkSheets(true);
    });
  }
            ...
            children: [
              Container(
                ...
                child: TabBar(
                  ...
                  tabs: [
                    TabBadge(
                        StatusMechanicalWorkSheet.pending, 'Đang cấp phép'),
                    TabBadge(StatusMechanicalWorkSheet.rePerformTask,
                        'Thực hiện lại công tác'),
                    TabBadge(StatusMechanicalWorkSheet.performTask,
                        'Thực hiện công tác'),
                    TabBadge(StatusMechanicalWorkSheet.waitingConfirm,
                        'Chờ xác nhận hoàn thành'),
                    TabBadge(StatusMechanicalWorkSheet.notFinished,
                        'Kết thúc chưa xong'),
                    TabBadge(
                        StatusMechanicalWorkSheet.confirmed, 'Đã xác nhận'),
                    TabBadge(StatusMechanicalWorkSheet.completed, 'Hoàn thành'),
                    TabBadge(StatusMechanicalWorkSheet.cancelled, 'Hủy'),
                  ],
                ),
              ),
              Expanded(
                child: TabBarView(
                  physics: ClampingScrollPhysics(),
                  children: [
                    MechanicalWorkSheetTabView(
                      status: StatusMechanicalWorkSheet.pending,
                    ),
                    MechanicalWorkSheetTabView(
                      status: StatusMechanicalWorkSheet.rePerformTask,
                    ),
                    MechanicalWorkSheetTabView(
                      status: StatusMechanicalWorkSheet.performTask,
                    ),
                    MechanicalWorkSheetTabView(
                      status: StatusMechanicalWorkSheet.waitingConfirm,
                    ),
                    MechanicalWorkSheetTabView(
                      status: StatusMechanicalWorkSheet.notFinished,
                    ),
                    MechanicalWorkSheetTabView(
                      status: StatusMechanicalWorkSheet.confirmed,
                    ),
                    MechanicalWorkSheetTabView(
                      status: StatusMechanicalWorkSheet.completed,
                    ),
                    MechanicalWorkSheetTabView(
                      status: StatusMechanicalWorkSheet.cancelled,
                    ),
                  ],
                ),
              ),
...
class MechanicalWorkSheetTabView extends StatefulWidget {
  final StatusMechanicalWorkSheet status;

  const MechanicalWorkSheetTabView({Key key, @required this.status})
      : super(key: key);

  @override
  MechanicalWorkSheetTabViewState createState() {
    return new MechanicalWorkSheetTabViewState();
  }
}

class MechanicalWorkSheetTabViewState
    extends State<MechanicalWorkSheetTabView> {
  @override
  Widget build(BuildContext context) {
    return Consumer<MechanicalWorkSheetModel>(
      builder: (_, model, __) => Column(
        children: [
          Container(
          ...
          Expanded(
            ...
            child: model.isLoading
                ...
                : _buildListDataItem(
                    context: context,
                    mechanicalWorkSheets: model.getDataByStatus(widget.status)),
          )),
        ],
      ),
    );
...
Widget _buildListDataItem({
  ...
  @required List<MechanicalWorkSheet> mechanicalWorkSheets,
}) {
  ...
  return Padding(
    ...
    child: RefreshIndicator(
      ...
      child: ListView.separated(
        itemCount: mechanicalWorkSheets.length,
        itemBuilder: (context, index) {
          return ListItemInfo(
            itemPairsData: [
              {
                'icon': 'ic_circle',
                'content': mechanicalWorkSheets[index].noiDung,
                'color': kWorkSheetStatusColor[EnumToString.convertToString(
                    mechanicalWorkSheets[index].getStatus())],
              },
              {
                'status': mechanicalWorkSheets[index].getStatusName(),
                'color': kWorkSheetStatusColor[EnumToString.convertToString(
                    mechanicalWorkSheets[index].getStatus())],
              },
              {
                'icon': 'ic_page',
                'content': 'Số phiếu : ${mechanicalWorkSheets[index].code}',
              },
              {
                'icon': 'ic_avatar',
                'content':
                    'Chỉ huy trực tiếp : ${mechanicalWorkSheets[index].tenChiHuy}',
              },
              {
                'icon': 'ic_clock',
                'content':
                    'Ngày lập phiếu : ${mechanicalWorkSheets[index].ngayLapPhieu.toDate(Constant.ddMMyyyy)}',
              },
              {
                'icon': 'ic_location',
                'content': 'Địa điểm : ${mechanicalWorkSheets[index].diaDiem}',
              },
              {
                'icon': 'ic_avatar',
                'content':
                    'Nội dung công việc : ${mechanicalWorkSheets[index].noiDung}',
              },
            ],
            onTap: () =>
                _showActionsDialog(context, mechanicalWorkSheets[index]),
            showActions: true,
          );
        },
    ...
    return Tab(
      child: Row(
        ...
        children: [
          ...
          Consumer<MechanicalWorkSheetModel>(
...
void _pushFilterScreen(BuildContext context) async {
  ...
  Provider.of<MechanicalWorkSheetModel>(context, listen: false).filter(result);
}

void _showActionsDialog(BuildContext context, MechanicalWorkSheet model) {
  ModalBottomSheetDialog modalBottomSheetDialog = ModalBottomSheetDialog(
    context: context,
    onTapListener: (item) async {
      if (item.key == '2') {
        Navigator.pushNamed(
            context, RouteList.mechanical_safety_work_sheet_detail,
            arguments:
                MechanicalWorkSheetArguments(mechanicalWorkSheet: model));
      } else if (item.key == '3') {
        Navigator.pushNamed(
            context, RouteList.mechanical_safety_work_sheet_add_member,
            arguments:
                MechanicalWorkSheetArguments(mechanicalWorkSheet: model));
      } else if (item.key == '4') {
        Navigator.pushNamed(
            context, RouteList.mechanical_safety_work_sheet_add_work_place,
            arguments:
                MechanicalWorkSheetArguments(mechanicalWorkSheet: model));
      } else if (item.key == '5') {
        ...
        if (result != null) {
          NotificationDialog.show(
              ...
              result.isSuccess() ? 'ic_success' : 'ic_error',
              result.isSuccess()
                  ? 'Hủy phiếu công tác CNH thành công'
                  : 'Đã xảy ra lỗi, Vui lòng thử lại');
          Provider.of<MechanicalWorkSheetModel>(context, listen: false)
              .getMechanicalWorkSheets(true);
        }
  ...
  data.add(ModalBottomSheetItemPair(key: "1", content: model.noiDung));
...
Future<StatusResponse> _showChangeDirectCommanderDialog(
  ...
  ListMemberResponse memberResponse =
      await Provider.of<MechanicalWorkSheetModel>(context, listen: false)
          .getDirectCommanderList(mechanicalWorkSheet.iDChiHuy);
                  ...
                  Expanded(
                    child: TextButton(
                      ...
                      onPressed: () async {
                        ...
                        final result =
                            await Provider.of<MechanicalWorkSheetModel>(context,
                                    listen: false)
                                .changeDirectCommander(
                          mechanicalWorkSheet.iD,
                          _selectedUser.iD,
                ...
                children: [
                  ...
                  Padding(
                    ...
                    child: DropdownSearch<ShiftLeaders>(
                      ...
                      items: mechanicalWorkSheet.users,
                  ...
                  Expanded(
                    child: TextButton(
                      ...
                      onPressed: () async {
                        ...
                        final result = await Provider.of<MechanicalWorkSheetModel>(
                            context,
                            listen: false)
                            .changeShiftLeader(
                          mechanicalWorkSheet.iD,
...
Future<StatusResponse> _showChangeAssignmentDialog(
  ...
  MechanicalWorkSheetModel mechanicalWorkSheetModel =
  Provider.of<MechanicalWorkSheetModel>(context, listen: false);
  await mechanicalWorkSheetModel.getMechanicalWorkSheetDetail(mechanicalWorkSheet.iD);

  if (mechanicalWorkSheetModel.detailData.thongTinChung.iDNguoiChoPhep != null &&
      mechanicalWorkSheetModel.detailData.thongTinChung.iDNguoiChoPhep > 0) {
    _selectedNguoiChoPhep = mechanicalWorkSheet.users.firstWhere(
            (user) =>
        mechanicalWorkSheetModel.detailData.thongTinChung.iDNguoiChoPhep == user.iD,
        orElse: () => null);
  }

  if (mechanicalWorkSheetModel.detailData.thongTinChung.iDNguoiChoPhepTaiCho != null &&
      mechanicalWorkSheetModel.detailData.thongTinChung.iDNguoiChoPhepTaiCho > 0) {
    _selectedNguoiChoPhepTC = mechanicalWorkSheet.users.firstWhere(
            (user) =>
        mechanicalWorkSheetModel.detailData.thongTinChung.iDNguoiChoPhepTaiCho ==
            user.iD,
        orElse: () => null);
  }
  ...
  final result = await showDialog(
    ...
    builder: (BuildContext context) => CustomTwoButtonDialog(
      ...
      title: Padding(
        ...
        child: Text(
          'Xác nhận hủy phiếu công tác CNH',
          textAlign: TextAlign.center,
          style: kMediumBlackTextStyle,
        ),
      ),
      content: Container(
        child: Padding(
          ...
          child: Text(
            'Bạn có đồng ý hủy phiếu công tác CNH ?',
            textAlign: TextAlign.center,
          ),
        ),
      ),
      ...
      onTapPositiveBtn: () async {
        final result =
            await Provider.of<MechanicalWorkSheetModel>(context, listen: false)
                .confirmCancel(mechanicalWorkSheet.iD);
        Navigator.of(context).pop(result);
      },
    ),
    barrierDismissible: false,
